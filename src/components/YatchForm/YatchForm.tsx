...
const YachtForm = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Prepare the payload—e.g., combine dimensions, generate crews array, etc.
    const yachtDataPayload = {
      ...formData,
    ...
    try {
      if (isEditMode && yachtId) {
        // Call updateYacht for edit mode
        await ownerAPI.updateYacht(yachtId, yachtDataPayload);
        // Optionally navigate back to the yacht detail page or a confirmation page
        navigate(`/yatch-details/${yachtId}`);
      } else {
        // For a new yacht listing, navigate to the review page
        navigate('/yatch-review', { 
          state: { 
            yachtData: yachtDataPayload,
            isEdit: false
          }
        });
      }
      ...
      setSubmitError(
        error instanceof Error 
          ? error.message 
          : 'Failed to submit yacht data. Please try again.'
      );
    }
  };
    ...
    if (isEditMode && yachtId) {
      const fetchYachtData = async () => {
        try {
          ...
          // @ts-ignore
          const data =response.yatch;
          console.log("data edit", data)
          // Assuming the API returns an object with similar keys as in formData.
          // If "dimension" is stored as a string like "LxWxH", split it:
          // let dimensionsObj = { length: '', width: '', height: '' };
          // if (data.dimension) {
          //   const parts = data.dimension.split('x');
          //   if (parts.length === 3) {
          //     dimensionsObj = {
          //       length: parts[0],
          //       width: parts[1],
          ...
          setFormData((prev) => ({
            ...
            price: {
              sailing: {
                peakTime: data.price?.sailing?.peakTime || 0,
                nonPeakTime: data.price?.sailing?.nonPeakTime || 0,
              },
              anchoring: {
                peakTime: data.price?.anchoring?.peakTime || 0,
                nonPeakTime: data.price?.anchoring?.nonPeakTime || 0,
              },
            },
            capacity: data.capacity || 0,
            // mnfyear: data.mnfyear || undefined,
          ...
          // Set selected location (using the existing locationOptions)
          const foundLocation = locationOptions
            .flatMap(group => group.options)
            .find(opt => opt.value === data.pickupat) || null;
          setSelectedLocation(foundLocation);

          // Pre-select add-on services based on data.addonServices
          if (data.addonServices && data.addonServices.length > 0) {
            const selected = data.addonServices.map((item: any) => ({
              label: item.service,
              value: item.service,
            }));
            setSelectedAddons(selected);
          ...
          // Pre-select packages based on data.packageTypes
          if (data.packageTypes && data.packageTypes.length > 0) {
            const selected = data.packageTypes.map((pkg: string) => {
              const option = packageOptions.find(opt => opt.value === pkg);
              return option ? option : { label: pkg, value: pkg };
            });
            setSelectedPackages(selected);
          }

          // Pre-set the DatePicker values if available
          if (data.availabilityFrom) {
            setStartTime(new Date(data.availabilityFrom));
          }
          if (data.availabilityTo) {
            setEndTime(new Date(data.availabilityTo));
          }
        } catch (err) {
          console.error("Failed to fetch yacht details for editing", err);
        }
      };
      fetchYachtData();
    }
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <div className={styles.yatchBox}>
        ...
        Showcase your yacht to a global audience by adding details, photos, and availability & start earning today!
        </div>
      </div>
      ...
      >
        {isSubmitting ? (isEditMode ? 'Updating...' : 'Creating...') : (isEditMode ? 'Update Yacht' : 'Confirm & Continue')}
      </button>
    </form>
  );
};
